...
def compare_columns(sheet, sheet_2, row, column1, column2):
    ...
    # Compare the values
    if value1 == value2:
        print(value1, value2, 'Same')
        return 'Same'
    else:
        print(value1, value2, 'No')
        return 'No'

pupil_position_grid_index = 0

folder_path = 'Autis - B'
result_path = 'Hasil/Hasil_Autis_B.txt'
...
sheet = workbook['Autis']
sheet_normal = workbook['Normal']
...
sheet.cell(row=row, column=col + 2, value='Analysis')
sheet.cell(row=row + 1, column=col + 2, value='Match Analysis')
sheet.cell(row=row + 2, column=col + 3, value='Match Result')
...
for row in range(start_row, end_row + 1):
    ...
    result = compare_columns(sheet_normal,sheet, row, col_normal, col + 1)
    
    # Write the result to the next column (D column in this case)
    sheet.cell(row=row, column=col + 2).value = result
...
count_cell = sheet.cell(row=start_row, column=col + 3)
count_cell.value = f'Same Count = {same_count}'
percentage_cell = sheet.cell(row=start_row + 1, column=col + 3)
